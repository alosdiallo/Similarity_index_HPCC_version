...
sub feeder(){
	...
	for ($x = 0; $x < 2;$x++){
			my $holder = $matrix[$x];
			open(OUT,">".$file_info."/"."c_result_".$x.".txt");
	...
	#			print "\n";
			...
			my $myTri = `/share/bin/R-2.14.1/bin/Rscript /home/dialloa/bin/triangle_gen.r /$file_info/c_result_0.txt $file_info`;
			chomp $myTri;
			my $p = 0;
			my $line = 0;
			my @triangle;
			...
			open(LIST,">".$file_info."/"."c_list_".$x.".txt");
			for($l=0; $l<$i; $l++){
				for($m=0; $m<$w; $m++){
	...
	open(CSISTAT,">>"."/home/dialloa/nearline/index_results"."/"."c_stats".".".$jobid_num[1]."."."txt");
	...
	my $spear = `/share/bin/R-2.14.1/bin/Rscript /home/dialloa/bin/spear.r /$file_info/c_list_0.txt /$file_info/c_list_1.txt`;
	chomp $spear;
	my @collection =  split(/\n/, $spear);
	...
	close CSISTAT;
	open(ENDER,">>"."/home/dialloa/nearline/index_results"."/"."CHECK_FILE_C.txt");
	print ENDER "1";
	close ENDER;
