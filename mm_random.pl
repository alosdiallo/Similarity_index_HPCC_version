...
sub feeder($$$){
	...
	for ($x = 0; $x < 2;$x++){
		my $holder = $matrix[$x];
		open(OUT,">".$file_info."/"."mm_result_".$x.".txt");
		...
		($sol_ref,$i,$w)=&mm(\@main_2D_array_result,\@row_array_result,$i_r,$file_info);
		...
		for($l=0; $l<$i; $l++){
			...
			my $myTri = `/share/bin/R-2.14.1/bin/Rscript /home/dialloa/bin/triangle_gen.r /$file_info/mm_result_0.txt $file_info`;
			chomp $myTri;
			my $p = 0;
			my $line = 0;
			my @triangle;
			...
			open(LIST,">".$file_info."/"."mm_list_".$x.".txt");
			for($l=0; $l<$i; $l++){
				for($m=0; $m<$w; $m++){
	...
	open(CSISTAT,">>"."/home/dialloa/nearline/index_results"."/"."mm_stats".".".$jobid_num[1]."."."txt");
	...
	my $spear = `/share/bin/R-2.14.1/bin/Rscript /home/dialloa/bin/spear.r /$file_info/mm_list_0.txt /$file_info/mm_list_1.txt`;
	chomp $spear;
	my @collection =  split(/\n/, $spear);
	...
	close CSISTAT;
close OUT;
	open(ENDER,">>"."/home/dialloa/nearline/index_results"."/"."CHECK_FILE_M.txt");
	print ENDER "1";
	close ENDER;
}
&feeder;
